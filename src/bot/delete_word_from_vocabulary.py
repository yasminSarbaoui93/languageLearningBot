...
def _ask_for_word_to_be_deleted(user_message, bot):
    ...
    bot: the bot object to send messages to the user
    """
    dictionary = get_user_dictionary(str(user_message.from_user.id))
    ...
    bot.register_next_step_handler(user_message, lambda user_message: _delete_word(user_message, bot, dictionary))


def _delete_word(user_message, bot, dictionary):    
    """
    ...
    base_language_code = dictionary.base_language
    try:
        words_deleted = delete_word(dictionary, word_to_be_deleted)
        if words_deleted:
